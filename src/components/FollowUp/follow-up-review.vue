                                            ...
                                            <div class="xl:col-span-2 space-y-6 relative">
                                                ...
                                                <div class="relative space-y-8 before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-0.5 before:bg-slate-200 dark:before:bg-slate-800">
                                                    ...
                                                    <div v-for="followup in followups" :key="followup.id" class="relative flex items-start gap-6 group">
                                                        ...
                                                        <div class="flex-1 bg-white dark:bg-slate-800/40 p-6 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm transition-all group-hover:shadow-lg group-hover:border-indigo-100 dark:group-hover:border-indigo-900/50 relative overflow-hidden">
    
                                                            <div class="absolute top-4 right-4 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform translate-y-[-10px] group-hover:translate-y-0">
                                                                <button @click="deleteFollowupContent(followup)" 
                                                                        class="flex items-center justify-center w-8 h-8 rounded-lg bg-red-50 dark:bg-red-900/20 text-red-500 hover:bg-red-500 hover:text-white transition-all duration-200 border border-red-100 dark:border-red-800/30 shadow-sm"
                                                            ...
                                                            <div class="flex justify-between items-start mb-5 pr-10">
                                                                <div>
                                                                    <h4 class="text-base font-bold text-slate-800 dark:text-slate-100 tracking-tight leading-tight">
                                                                        {{ followup.title }}
                                                                    </h4>
                                                                    <div class="flex items-center gap-2 mt-2">
                                                                        <span class="inline-flex items-center text-[10px] font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest bg-indigo-50 dark:bg-indigo-900/30 px-2.5 py-1 rounded-md border border-indigo-100/50 dark:border-indigo-500/20">
                                                                            <i class="fa-solid fa-tag mr-1.5 text-[8px]"></i>
                                                                            {{ followup.contact_type }}
                                                                        </span>
                                                                    </div>
                                                                </div>
                                                                <div class="text-right">
                                                                    <span class="text-[11px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-tighter bg-slate-50 dark:bg-slate-900/50 px-2 py-1 rounded-md">
                                                                        {{ new Date(followup.last_contacted_at).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) }}
                                                                    </span>
                                                                </div>
                                                            </div>

                                                            <div class="flex justify-between items-center p-4 mb-5 rounded-xl border border-slate-200/60 dark:border-slate-700/50 bg-slate-50/50 dark:bg-slate-900/60 shadow-inner">
                                                                <div class="space-y-1">
                                                                    ...
                                                                    <div class="flex items-baseline gap-1">
                                                                        ...
                                                                        <h4 class="text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tighter">
                                                                            {{ followup.deal_amount || '0.00' }}
                                                                        </h4>
                                                                    </div>
                                                                </div>
                                                                ...
                                                                <div class="flex flex-col items-end">
                                                                    <span :class="[
                                                                        'text-[10px] font-black px-3 py-1.5 rounded-lg uppercase tracking-wider border shadow-sm transition-all',
                                                                        followup.is_converted
                                                                            ? 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400 dark:border-emerald-500/20'
                                                                            : 'bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-500/10 dark:text-amber-400 dark:border-amber-500/20'
                                                                    ]">
                                                                        <i :class="followup.is_converted ? 'fa-solid fa-check-double mr-1.5' : 'fa-solid fa-clock mr-1.5'"></i>
                                                                        {{ followup.is_converted ? 'Converted' : 'Pending' }}
                                                                    </span>
                                                                </div>
                                                            </div>

                                                            <div class="space-y-3 bg-white/40 dark:bg-slate-900/20 p-4 rounded-xl border border-slate-100 dark:border-slate-800/60">
                                                                <div class="flex items-start gap-4">
                                                                    <div class="flex-shrink-0 flex items-center justify-center w-8 h-8 rounded-lg bg-white dark:bg-slate-800 text-indigo-500 shadow-sm border border-slate-100 dark:border-slate-700">
                                                                        <i class="fa-solid fa-note-sticky text-xs"></i>
                                                                    </div>
                                                                    <div class="flex-1">
                                                                        <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Follow-up Note</span>
                                                                ...
                                                                <div class="h-px w-full bg-gradient-to-r from-transparent via-slate-200 dark:via-slate-700 to-transparent my-1"></div>

                                                                <div class="flex items-start gap-4">
                                                                    <div class="flex-shrink-0 flex items-center justify-center w-8 h-8 rounded-lg bg-white dark:bg-slate-800 text-emerald-500 shadow-sm border border-slate-100 dark:border-slate-700">
                                                                        <i class="fa-solid fa-circle-check text-xs"></i>
                                                                    </div>
                                                                    <div class="flex-1">
                                                                        <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Final Outcome</span>
                                                            ...
                                                            <div class="flex items-center justify-between mt-5 pt-4 border-t border-slate-100 dark:border-slate-800">
                                                                <div class="flex items-center gap-2">
                                                                    <div class="w-6 h-6 rounded-full bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center">
                                                                        <i class="fa-solid fa-user text-[10px] text-indigo-600 dark:text-indigo-400"></i>
                                                                    </div>
                                                                    <span class="text-[11px] font-bold text-slate-500 dark:text-slate-400 italic">By {{ followup.user?.name }}</span>
                                                                </div>
                                                                
                                                                <div class="flex items-center gap-2">
                                                                    <span class="px-2.5 py-1 rounded-md text-[9px] font-black uppercase flex items-center gap-1.5 shadow-sm border"
                                                                        :class="getPriorityClass(followup.priority)">
                                                                        <span class="w-1.5 h-1.5 rounded-full bg-current opacity-80"></span>
                                                                        {{ followup.priority }}
                                                                    </span>

                                                                    <span class="px-2.5 py-1 rounded-md text-[9px] font-black uppercase shadow-sm border"
...
async function fetchedCustomers(page = 1) {
    ...
    try {
        ...
        const res = await api.get('/follow-up/review', { params });
...
const deleteFollowupContent = async (followup) => {
    if (!confirm('Are you sure you want to delete this follow-up?')) return;

    try {
        const res = await api.delete(`/follow-up/${followup.id}`);
        await fetchFollowupHistory(followup.id);
        successMsg.value = res.data.message || 'Deleted successfully';
    } catch (e) {
